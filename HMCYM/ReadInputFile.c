...
/*
 * read in parameters from file HMCYM.txt
 */
int ReadInputFile(int *lnum, int *size, double *eps, double *lacc, double *uacc, int *therm, int *steps, int *step, char *pathout, int *EVstore,
		int *nummat, double *mass, int *start, int *compC, int *compD, int *compEV, int *compComm, int *compTraces
		, int *compTraceX2, int *compGdd)
{
	FILE *read;
	char config[1000];
	...
	while(fgets(config, 1000, read)!= NULL)
	{
		...
		//include computation of EV TraceX2
		if(!strcmp(config, "compTraceX2\n"))
		{
			check = fgets(config, 1000, read);
			*compTraceX2= atoi(config);
		}
		//include computation of EV Gdd
		if(!strcmp(config, "compGdd\n"))
		{
			check = fgets(config, 1000, read);
			*compGdd= atoi(config);
		}
