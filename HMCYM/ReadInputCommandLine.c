...
/*
 * read in parameters from command line supplied by python-script HMCYM.py
 */
		...
		int *compTraces, int *compTraceX2, int *compGdd)
{
	...
	for(argNum = 1; argNum + 2 < argc; argNum++)
	{
		if(!strcmp(argv[argNum], "-par"))
		{
			...
			if(!strcmp(argv[argNum + 1], "compTraceX2"))
			{
				*compTraceX2 = atoi(argv[argNum + 2]);
			}
			if(!strcmp(argv[argNum + 1], "compGdd"))
			{
				*compGdd = atoi(argv[argNum + 2]);
			}
			argNum += 2;
		}
	}
